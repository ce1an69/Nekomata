...
<p align="center">
  <a href="README.md"><img alt="Language" src="https://img.shields.io/badge/lang-English-b4befe?style=flat-square"></a>
...
| **三种运行模式** | TUI（默认）/ CLI / Desktop，按场景选择入口 |
...
nekomata-tarot
nekomata-tarot -c
nekomata-tarot -c -q "今天运势如何？" -S past_present_future -y
nekomata-tarot --desktop
```

| 命令 | 模式 | 说明 |
| --- | --- | --- |
| `nekomata-tarot` | TUI | 默认模式，完整终端界面 |
| `nekomata-tarot -c` | CLI | 纯命令行交互 |
| `nekomata-tarot -c -q ... -S ... -y` | CLI | 单行占卜，可用于脚本 |
| `nekomata-tarot --desktop` | Desktop | PyWebView 原生桌面窗口 |
...
src/nekomata/
├── app.py         入口调度器
├── desktop.py     桌面模式（PyWebView）
├── core/          核心代码（card / spread / ai / storage / i18n / render）
├── cli/           CLI 模式
