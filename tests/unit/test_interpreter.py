...
def test_openai_interpret_success():
    mock_resp = _mock_urlopen({"choices": [{"message": {"content": "AI interpretation result"}}]})
...
def test_openai_sends_auth_header():
    mock_resp = _mock_urlopen({"choices": [{"message": {"content": "ok"}}]})
...
def test_openai_interpret_failure_raises():
    ...
    with patch(
        "nekomata.core.ai.interpreter.urllib.request.urlopen", side_effect=urllib.error.URLError("connection refused")
    ):
...
def test_openai_empty_content_retryable():
    mock_resp = _mock_urlopen({"choices": [{"message": {"content": "   "}}]})
    with patch("nekomata.core.ai.interpreter.urllib.request.urlopen", return_value=mock_resp):
        interp = OpenAIInterpreter(model="test-model")
        with pytest.raises(InterpretationError) as exc_info:
            interp.interpret(make_drawn_cards(1), "test")
...
def test_interpret_stream_failure_raises():
    ...
    with patch("nekomata.core.ai.interpreter.urllib.request.urlopen", side_effect=urllib.error.URLError("timeout")):
        interp = OpenAIInterpreter(model="test-model")
        with pytest.raises(InterpretationError) as exc_info:
